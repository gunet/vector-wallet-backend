...
app.use(async (req, res, next) => {
	...
	if (req.cookies["did"] && req.cookies["appToken"] && req.cookies["did"] != "undefined" && req.cookies["appToken"] != "undefined") {
		global.user.did = req.cookies["did"];
		global.user.appToken = req.cookies["appToken"]
		console.log("User = ",global.user)
	}
	else {
		await registerUser();
		console.log("Registered user")
		res.cookie("did", global.user.did);
		res.cookie("appToken", global.user.appToken)
	}
...
app.post('/select-vc', async (req, res) => {
	...
	axios.post(walletBackendUrl + "/presentation/generate/authorization/response",
			...
			res.render('error', { title: "Error", status: 500 })
		});
})
