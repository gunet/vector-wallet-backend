...
async function getUserByDID(did: string): Promise<Result<UserEntity, GetUserErr>> {
	try {
		...
		res.keys = res.keys.toString(); // from Buffer to string
		if (!res) {
...
async function getUserByUsername(username: string): Promise<Result<UserEntity, GetUserErr>> {
	try {
		...
		res.keys = res.keys.toString(); // from Buffer to string
		res.fcmToken = res.fcmToken ? res.fcmToken.toString() : ""; // from Buffer to string
...
async function getAllUsers(): Promise<Result<UserEntity[], GetUserErr>> {
	try {
		...
		res = res.map((r) => {
			r.keys = r.keys.toString(); // from Buffer to string
			r.fcmToken = r.fcmToken ? r.fcmToken.toString() : ""; // from Buffer to string
			return r;
		})
